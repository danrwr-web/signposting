...
## Reviewing by age group

Every symptom in the toolkit is **age-group specific**. The same symptom name — for example, *Cough* — exists as up to three separate versions:

- **Under 5** (shown with a blue badge)

- **5–17** (shown with a green badge)

- **Adult** (shown with a purple badge)

Each version is written, edited, and clinically approved **separately**, because safe advice for a feverish toddler is rarely the same as for an adult.

When carrying out a clinical review:

- Check the **age badge** next to the symptom name before approving — it appears in the review list, the preview panel, and at the top of the symptom page.

- Use the **age group filter** in Clinical Review to work through one age band at a time (for example, approve all Adult symptoms first, then the children's versions).

- Make sure the wording is **specific to that age group**. Avoid writing a single all-ages instruction into one version — reception teams see the version that matches the patient's age, so generic wording causes confusion at the front desk.

- When editing, the editor shows a reminder of which age group the version applies to.

If you find symptoms that were written for all ages by mistake, edit each age-group version so the wording matches its band, then use **Request re-review** so your Clinical Lead can re-approve them. See [Symptom Library](/features/symptom-library) for how base symptoms, overrides, and custom symptoms fit together.

### Surgeries without age bands

A surgery can instead opt out of age bands entirely (a per-surgery option enabled by the Signposting Toolkit team — see [single all-ages mode](/features/symptom-library#single-all-ages-mode-per-surgery)). In that mode, clinical review covers **one entry per symptom**, and the reviewer must check that the single set of instructions safely covers red flags and escalation advice for **all ages** — babies and young children included — with any age-specific advice stated explicitly in the wording.
...
### What sends a symptom back for review

Any AI-assisted change to a symptom returns it to **Pending**, including:

- applying an improved-wording suggestion — whether you accepted the AI's draft as-is or edited it first
- running AI surgery-specific customisation
- **saving an AI smart visual** for the symptom

Smart visuals are worth calling out. A smart visual does not change a single word of the instructions — but the AI decides what counts as a red flag, which decisions are routing options, and what order everything appears in. That reinterpretation is what reception staff would act on, so it is reviewed like any other clinical content: staff continue to see the standard instructions until a clinician approves the symptom. Practice admins can see the pending visual in the meantime, badged as awaiting approval.

Removing a smart visual only takes a view away, so it does not trigger a new review.
