...
This feature is controlled by the `ai_handbook_visuals` feature flag. System administrators (superusers) always have access regardless of the flag, so they can test and demo the feature on any practice; practice staff see saved visuals only once the flag is enabled for their surgery.
