...
Smart visuals turn a Handbook item into a polished, infographic-style view — contact cards, numbered process steps, checklists, team and buddy-group cards with working-day chips, role cards, key-fact tiles and warning callouts — rendered as normal app content, so the text stays selectable, accessible and readable on any screen size. This feature is controlled by the `ai_handbook_visuals` feature flag (contact us to enable it). System administrators always have access, so they can try the feature on any practice before it is switched on.
...
Once a visual is saved, staff viewing the item get a **Standard / Smart visual** toggle and can switch between the two views at any time. The visual opens by default while it is up to date.
