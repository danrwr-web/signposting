...
Anyone who can edit a handbook page — admins, or standard users with per-item editing permission — can add images to the **Intro text** and **Additional notes** sections. Click the image button in the editor toolbar, choose a **JPG or PNG file up to 2MB**, and optionally add alt text describing the image for screen readers (leave it blank if the image is purely decorative). The image is inserted where your cursor is and appears immediately.

Good uses include floor plans, annotated screenshots of practice systems, equipment photos, and process diagrams. A few things worth knowing:

- Images are **private to your practice**: they are only served to signed-in members of your surgery, and images on a page in a restricted category follow that category's visibility rules.
- Images scale down automatically to fit the page (and the reader's screen).
- **Anyone reading the page can click an image to view it full-screen**, then zoom in and drag to move around — useful when a floor plan or annotated screenshot is too detailed to read at page size. Press `Esc`, click the background, or use the close button to return to the page.

### Attachments and PDF documents

Every handbook item has an **Attachments** section where anyone who can edit the item can attach supporting documents in two ways:

- **Link** — paste a URL to a document that lives elsewhere (a shared drive folder, an intranet page, a Word document).
- **Upload PDF** — upload a **PDF file up to 4MB** directly into the handbook. The document is stored with your practice's handbook content and opens in the browser's built-in PDF viewer.

Uploaded PDFs are **private to your practice**, just like images: they are only served to signed-in members of your surgery, and documents attached to a page in a restricted category follow that category's visibility rules. Link attachments, by contrast, point wherever the URL goes — access to those is controlled by the destination, not by the toolkit.

You can also link to an uploaded PDF from within a page's **Intro text** or **Additional notes**: upload it as an attachment first, copy its address (right-click the "Uploaded PDF" link and copy the link address), then use the link button in the editor toolbar and paste it in.
...
Smart visuals turn a Handbook item into a polished, infographic-style view — contact cards, numbered process steps, checklists, team and buddy-group cards with working-day chips, role cards, key-fact tiles and warning callouts — rendered as normal app content, so the text stays selectable, accessible and readable on any screen size. Generating visuals is controlled by the `ai_handbook_visuals` feature flag (contact us to enable it); system administrators always have access, so they can try the feature on any practice before it is switched on. A visual that has been saved is visible to all staff regardless of the flag.

### Generating a visual

Anyone who can edit an item (admins, or standard users with per-item editing permission) will see a **Generate smart visual** button when viewing it. Clicking it sends the item's content to the AI, which chooses the most suitable visual treatments from a fixed set of layouts — it cannot invent free-form designs, colours or images, and it is instructed to use only facts present in the page.

The result appears as a clearly marked **preview**: nothing is saved until you review it and click **Save visual**. While previewing you can:

- **Reorder or remove sections** using the controls on each section (a visual always keeps at least one section)
- **Refine with guidance** — type a short instruction such as "group by site" or "make the warning more prominent" and regenerate; the AI applies your steer within the same fixed layout rules. Guidance shapes the *structure* — text styling is applied automatically by the app (phone numbers, for example, are always shown bold and tappable to dial)
- **Regenerate** for a fresh arrangement, or **discard** the preview entirely

When you regenerate an existing visual, the AI is shown the previous layout and asked to keep the same structure wherever the updated content still supports it — so small content edits don't reshuffle the whole visual.

### What staff see

Once a visual is saved, everyone viewing the item — staff, editors and admins alike — gets a **Standard / Smart visual** toggle and can switch between the two views at any time. The visual opens by default while it is up to date, and the standard view (including inline editing of LIST rows for those with permission) is always one click away.

A **Print visual** button prints just the visual (with the page title), formatted for paper — handy for pinning a rota or buddy-system sheet up at the desk.

### The page content stays the source of truth

A smart visual is a presentation of the page — the normal page content remains the single source of truth, and editing an item never changes its visual (or vice versa). If the underlying content is edited after a visual was generated, the visual is **automatically hidden from everyone** — staff simply see the standard view again — until an editor regenerates it. Editors viewing the item see a notice explaining that the visual is hidden, with a one-click regenerate, and are prompted to regenerate immediately after saving an edit to a page that has a visual. This guarantees an outdated visual can never be shown alongside newer page content. Saving and removing visuals is recorded in the [audit trail](#audit-trail).
...
Page content and additional notes are written in the rich-text editor, which supports section headings, bold, italic, underline, NHS-palette text colours, background highlights, bullet and numbered lists, and links (web addresses and e-mail). Content pasted from Word or Google Docs is cleaned up automatically.
