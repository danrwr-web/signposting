...
- The toolkit shows a notice when content is still awaiting clinical review: a prominent warning when many symptoms are pending (for example, in a newly set-up surgery), or a small one-line note when only a few recently added or edited symptoms are waiting. Everyone sees this, including admins. Follow the guidance but confirm with a clinician if worried.
