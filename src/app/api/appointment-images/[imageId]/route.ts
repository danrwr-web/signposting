...
export async function GET(
  ...
  try {
    ...
    const image = await prisma.appointmentImage.findUnique({
      where: { id: imageId },
    ...
    // revalidate on every use, so the permission gate above re-runs and a
    ...
    console.error('Error serving appointment image:', error)
