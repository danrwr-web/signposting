...
function validationError(error: z.ZodError): ActionResult<never> {
  return {
    ok: false,
    error: { code: 'VALIDATION_ERROR', message: 'Invalid input.', fieldErrors: zodFieldErrors(error) },
  }
}

async function withWriteAccess<T>(
  input: unknown,
  schema: z.ZodSchema<{ surgeryId: string }>,
  handler: (data: z.infer<typeof schema>, gate: { surgeryId: string; userId: string; isSuperuser: boolean }) => Promise<ActionResult<T>>
  ...
  return handler(parsed.data, gate.data)
}

async function withViewAccess<T>(
  input: unknown,
  schema: z.ZodSchema<{ surgeryId: string }>,
  handler: (data: z.infer<typeof schema>, gate: { surgeryId: string }) => Promise<ActionResult<T>>
): Promise<ActionResult<T>> {
  const parsed = schema.safeParse(input)
  if (!parsed.success) return validationError(parsed.error)
...
async function withItemEditCheck<T>(
  opts: { surgeryId: string; itemId: string; userId: string; isSuperuser: boolean },
  handler: () => Promise<ActionResult<T>>
): Promise<ActionResult<T>> {
  const allowed = await canEditItem(opts)
  if (!allowed) {
...
export async function renameAdminToolkitCategory(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, renameCategoryInput, async (data, gate) => {
    const { surgeryId, categoryId, name } = data
...
export async function deleteAdminToolkitCategory(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, deleteCategoryInput, async (data, gate) => {
    const { surgeryId, categoryId } = data
...
export async function createAdminToolkitPageItem(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, createPageItemInput, async (data, gate) => {
    const { surgeryId, title, categoryId, contentHtml, warningLevel, lastReviewedAt } = data
...
export async function createAdminToolkitItem(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, createItemInput, async (data, gate) => {
    const { surgeryId, type, title, categoryId, contentHtml, warningLevel, lastReviewedAt } = data
...
export async function updateAdminToolkitPageItem(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, updatePageItemInput, async (data, gate) => {
    const { surgeryId, itemId } = data
    return withItemEditCheck(
      { surgeryId, itemId, userId: gate.userId, isSuperuser: gate.isSuperuser },
      async () => {
        ...
        const cleanedHtml = sanitizeHtml(data.contentHtml || '')
        const next = {
          title: data.title,
        ...
        await prisma.$transaction(async (tx) => {
          await tx.adminItem.update({
            where: { id: itemId },
            data: { ...next, updatedByUserId: gate.userId },
          })
          await tx.adminHistory.create({
            data: {
              surgeryId,
...
export async function updateAdminToolkitItem(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, updateItemInput, async (data, gate) => {
    const { surgeryId, itemId } = data
    return withItemEditCheck(
      { surgeryId, itemId, userId: gate.userId, isSuperuser: gate.isSuperuser },
      async () => {
        ...
        const nextContentHtml = existing.type === 'PAGE' ? sanitizeHtml(data.contentHtml ?? existing.contentHtml ?? '') : existing.contentHtml
        const next = {
          title: data.title,
          categoryId: data.categoryId ?? null,
          warningLevel: data.warningLevel ?? null,
        ...
        await prisma.$transaction(async (tx) => {
          await tx.adminItem.update({ where: { id: itemId }, data: { ...next, updatedByUserId: gate.userId } })
          await tx.adminHistory.create({
            data: {
              surgeryId,
              adminItemId: itemId,
...
export async function createAdminToolkitListColumn(input: unknown): Promise<ActionResult<{ id: string }>> {
  return withWriteAccess(input, createListColumnInput, async (data, gate) => {
    const { surgeryId, itemId, label, fieldType } = data
    return withItemEditCheck(
      { surgeryId, itemId, userId: gate.userId, isSuperuser: gate.isSuperuser },
      async () => {
        ...
        const created = await prisma.$transaction(async (tx) => {
          ...
          await tx.adminItem.update({ where: { id: itemId }, data: { updatedByUserId: gate.userId } })
          await tx.adminHistory.create({
            data: {
              surgeryId,
              adminItemId: itemId,
...
export async function upsertAdminToolkitPinnedPanel(input: unknown): Promise<ActionResult<{ surgeryId: string }>> {
  return withWriteAccess(input, upsertPinnedPanelInput, async (data, gate) => {
    const { surgeryId, taskBuddyText, postRouteText } = data
...
export async function getAdminToolkitOnTakeWeekValue(input: unknown): Promise<ActionResult<{ gpName: string | null }>> {
  return withViewAccess(input, getOnTakeWeekInput, async (data) => {
    const { surgeryId, weekCommencingIso } = data
    const weekCommencingUtc = new Date(`${weekCommencingIso}T00:00:00.000Z`)
