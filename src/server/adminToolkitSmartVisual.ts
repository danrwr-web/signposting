...
7. "people" — { "type": "people", "title"?: string (<=80), "groups": [{ "title": string (<=80), "theme"?: theme, "note"?: string (<=160), "members": [{ "name": string (<=80), "tag"?: string (<=40), "days"?: ["Mon"|"Tue"|"Wed"|"Thu"|"Fri"|"Sat"|"Sun"], "facts"?: [{ "label": string (<=40), "value": string (<=140) }] (max 4), "note"?: string (<=160) }] (1-12) }] (1-8) }
   Use for: teams, buddy systems, rotas, staff lists — whenever the content describes PEOPLE with attributes. Group members by the natural grouping in the source (buddy group, team, site, role). "tag" is a short badge next to the name; when the source gives a colour attribute, put JUST the colour name in "tag" (the app automatically renders colour-named tags in that colour). Working days MUST go in "days" as three-letter abbreviations, never in prose. Other attributes (assigned nurse, care home, ward round day) go in "facts" as short label/value pairs.
