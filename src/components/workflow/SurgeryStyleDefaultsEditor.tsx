...
interface Props {
  surgeryId: string
  styleDefaults: SurgeryStyleDefault[]
  isSuperuser: boolean
}
...
export default function SurgeryStyleDefaultsEditor({ surgeryId, styleDefaults, isSuperuser }: Props) {
  const router = useRouter()
  const [editingDefaults, setEditingDefaults] = useState<Map<WorkflowNodeType, SurgeryStyleDefault>>(() => {
    const map = new Map<WorkflowNodeType, SurgeryStyleDefault>()
  ...
  const handleSave = async (nodeType: WorkflowNodeType) => {
    ...
    setSaving(nodeType)
    try {
      const response = await fetch(`/s/${surgeryId}/workflow/admin/style-defaults`, {
        method: 'POST',
        headers: {
      ...
      console.error('Error saving surgery style default:', error)
  ...
  const handleReset = async (nodeType: WorkflowNodeType) => {
    ...
    try {
      const response = await fetch(
        `/s/${surgeryId}/workflow/admin/style-defaults?nodeType=${nodeType}`,
        {
          method: 'DELETE',
        }
      )
      ...
      console.error('Error resetting surgery style default:', error)
  ...
  const handleResetAll = async () => {
    if (!confirm('Are you sure you want to reset ALL surgery style defaults? This cannot be undone.')) {
      return
    }
    setResetting('INSTRUCTION' as WorkflowNodeType) // Use a placeholder for "all"
    try {
      const response = await fetch(
        `/s/${surgeryId}/workflow/admin/style-defaults`,
        {
          method: 'DELETE',
        }
      )
      ...
      console.error('Error resetting all surgery style defaults:', error)
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        ...
        >
          {resetting !== null ? 'Resetting...' : 'Reset All Surgery Defaults'}
        </button>
      </div>
    </div>
  )
