...
export default function WorkflowDiagramClient({
  ...
  // Add debug edge ID to edges for debug overlay (development only)
  const edgesForRender = useMemo(() => {
    if (!ENABLE_EDGE_DEBUG || !debugEdgeId) return edges
    return edges.map((edge) => ({
      ...edge,
      data: { ...(edge.data || {}), debugEdgeId },
    }))
  }, [edges, debugEdgeId, ENABLE_EDGE_DEBUG])
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-6 h-[800px]">
        ...
        <div className="relative flex-1 bg-white rounded-lg border border-gray-300 overflow-hidden">
          ...
          <ReactFlow
            nodes={nodes}
            edges={ENABLE_EDGE_DEBUG ? edgesForRender : edges}
            nodeTypes={nodeTypes}
            edgeTypes={edgeTypes}
